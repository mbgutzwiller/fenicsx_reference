...
for i, grid_size in zip(range(len(grid_sizes)), grid_sizes):
    ...
    a_values = a.x.array
    for i, xi in enumerate(x):
        ax = -16. * np.pi**2 * np.sin(4. * np.pi * xi[0]) * np.sin(2. * np.pi * xi[1]) * np.sin(4. * np.pi * (current_time - 0.1))
        ay = -16. * np.pi**2 * np.sin(4. * np.pi * xi[0]) * np.sin(2. * np.pi * xi[1]) * np.sin(4. * np.pi * (current_time + 0.3))
        a_values[2*i] = ax
        a_values[2*i + 1] = ay
    a.x.scatter_forward()
