...
if comm.rank == 0:
    ...
    # dim1 = 41  # number of rows
    # num_nodes = dim1 ** 2

    # u_array = u.x.array
    # assert u_array.size == 2 * num_nodes, "Mismatch in array size vs grid dimensions"

    # # Extract ux and uy from solution vector (only last step here)
    # ux_num = u_array[::2].reshape((dim1, dim1))  # x displacements
    # uy_num = u_array[1::2].reshape((dim1, dim1))  # y displacements

    # Get the dimensions (assumed square here)
    # This gives you the coordinates of each DoF, in the same order as u.x.array
    # Step 1: Get dof coordinates (drop z)
    # Get dof coordinates, strip z-dimension
    ...
    # Sanity check
    print(len(node_coords), len(ux), len(uy))  # should all be 1681
