...
kinetic_energy_density = 0.5 * ufl.inner(v, v)
...
for i, xi in enumerate(x):
    u_values[2*i] = np.sin(4*np.pi*xi[0]) * np.sin(2*np.pi*xi[1]) * np.sin(4.*np.pi*(0.-0.1))
    u_values[2*i+1] = np.sin(4*np.pi*xi[0]) * np.sin(2*np.pi*xi[1]) * np.sin(4.*np.pi*(0.+0.3))
...
# --- Compute a = F / M using PETSc's pointwiseDivide ---
...
while float(t.value) < total_time:
    # Update imposed boundary conditions (if needed)
    right_ux_value = float(U_imp.subs({t_sp: t.value}))
    # (Set right, left, top, bottom BCs as needed; here we assume they remain zero)
    ...
    # # Step 2: u_new <- u_new + dt * v
    # u_new.x.petsc_vec.axpy(dt, v.x.petsc_vec)
    # # Step 3: u_new <- u_new + 0.5 * dt^2 * a
    # u_new.x.petsc_vec.axpy(dt2, a.x.petsc_vec)
    # # Final step: sync ghost values
    # u_new.x.scatter_forward()
    ...
    for i, xi in enumerate(mesh.geometry.x):
        b_values[2*i] = _c_mu*(8.*np.pi**2.*np.cos(4.*np.pi*xi[0])*np.cos(2.*np.pi*xi[1])*np.sin(4.*np.pi*(t.value + 3./10.)) + 16.*np.pi**2.*np.sin(4.*np.pi*xi[0])*np.sin(2.*np.pi*xi[1])*np.sin(4.*np.pi*(t.value - 1./10.))) - _c_mu*(8.*np.pi**2.*np.cos(4.*np.pi*xi[0])*np.cos(2.*np.pi*xi[1])*np.sin(4.*np.pi*(t.value + 3./10.)) - 4.*np.pi**2.*np.sin(4.*np.pi*xi[0])*np.sin(2.*np.pi*xi[1])*np.sin(4.*np.pi*(t.value - 1./10.))) - _c_k ** 2*(8.*np.pi**2.*np.cos(4.*np.pi*xi[0])*np.cos(2.*np.pi*xi[1])*np.sin(4.*np.pi*(t.value + 3./10.)) - 16.*np.pi**2.*np.sin(4.*np.pi*xi[0])*np.sin(2.*np.pi*xi[1])*np.sin(4.*np.pi*(t.value - 1./10.))) - 16.*np.pi**2.*np.sin(4.*np.pi*xi[0])*np.sin(2.*np.pi*xi[1])*np.sin(4.*np.pi*(t.value - 1./10.))
        b_values[2*i+1] = _c_mu*(8.*np.pi**2.*np.cos(4.*np.pi*xi[0])*np.cos(2.*np.pi*xi[1])*np.sin(4.*np.pi*(t.value - 1./10.)) + 4.*np.pi**2.*np.sin(4.*np.pi*xi[0])*np.sin(2.*np.pi*xi[1])*np.sin(4.*np.pi*(t.value + 3./10.))) - _c_k ** 2*(8.*np.pi**2.*np.cos(4.*np.pi*xi[0])*np.cos(2.*np.pi*xi[1])*np.sin(4.*np.pi*(t.value - 1./10.)) - 4.*np.pi**2.*np.sin(4.*np.pi*xi[0])*np.sin(2.*np.pi*xi[1])*np.sin(4.*np.pi*(t.value + 3./10.))) - _c_mu*(8.*np.pi**2.*np.cos(4.*np.pi*xi[0])*np.cos(2.*np.pi*xi[1])*np.sin(4.*np.pi*(t.value - 1./10.)) - 16.*np.pi**2.*np.sin(4.*np.pi*xi[0])*np.sin(2.*np.pi*xi[1])*np.sin(4.*np.pi*(t.value + 3./10.))) - 16.*np.pi**2.*np.sin(4.*np.pi*xi[0])*np.sin(2.*np.pi*xi[1])*np.sin(4.*np.pi*(t.value + 3./10.))
    b.x.scatter_forward()
