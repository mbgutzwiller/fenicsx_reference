...
if comm.rank == 0:
    t_sampling = np.linspace(timestepping_parameters['initial_time'], 
                             ...
                             timestepping_parameters['total_steps'],
                             ) 
...
# # Locate all boundary facets (already defined in your code)
# all_boundary_facets = np.hstack([top_facets, bottom_facets, left_facets, right_facets])

# # Find the degrees of freedom associated with those facets (for vector space)
...
# # Apply it to all components of the vector function space
# bc_u = dolfinx.fem.dirichletbc(zero_disp, all_boundary_dofs, V_t)

# # Apply same BC to velocity and acceleration
# bcs_u = [bc_u]
# bcs_v = [bc_u]
# bcs_a = [bc_u]
...
# Update body force b based on t0 (if needed)
b_values = b.x.array
...
# Initial acceleration = M⁻¹ F
a.x.array[:] = F_init.array[:] / M_lumped.array[:]
...
while float(t.value) < total_time:
    ...
    # We'll need this later for nonzero dirichlet BC
    ...
    # left_vx.value = float(V_imp.subs({t_sp:t.value}))
    # left_ax.value = float(A_imp.subs({t_sp:t.value}))
    # left_ux.value = float(U_imp.subs({t_sp:t.value}))

    # top_vx.value = float(V_imp.subs({t_sp:t.value}))
    # top_ax.value = float(A_imp.subs({t_sp:t.value}))
    # top_ux.value = float(U_imp.subs({t_sp:t.value}))

    # bottom_ux.value = float(U_imp.subs({t_sp:t.value}))
    # bottom_vx.value = float(V_imp.subs({t_sp:t.value}))
    # bottom_ax.value = float(A_imp.subs({t_sp:t.value}))
    ...
    for i, xi in enumerate(x):
        ...
        b_values[2*i + 1] = _c_mu ** 2*(8.*np.pi**2.*np.cos(4.*np.pi*xi[0])*np.cos(2.*np.pi*xi[1])*np.sin(4.*np.pi*(t.value - 1./10.)) + 4.*np.pi**2.*np.sin(4.*np.pi*xi[0])*np.sin(2.*np.pi*xi[1])*np.sin(4.*np.pi*(t.value + 3./10.))) - _c_k ** 2*(8.*np.pi**2.*np.cos(4.*np.pi*xi[0])*np.cos(2.*np.pi*xi[1])*np.sin(4.*np.pi*(t.value - 1./10.)) - 4.*np.pi**2.*np.sin(4.*np.pi*xi[0])*np.sin(2.*np.pi*xi[1])*np.sin(4.*np.pi*(t.value + 3./10.))) - _c_mu ** 2*(8.*np.pi**2.*np.cos(4.*np.pi*xi[0])*np.cos(2.*np.pi*xi[1])*np.sin(4.*np.pi*(t.value - 1./10.)) - 16.*np.pi**2.*np.sin(4.*np.pi*xi[0])*np.sin(2.*np.pi*xi[1])*np.sin(4.*np.pi*(t.value + 3./10.))) - 16.*np.pi**2.*np.sin(4.*np.pi*xi[0])*np.sin(2.*np.pi*xi[1])*np.sin(4.*np.pi*(t.value + 3./10.))
        # b_values[2*i + 1] = np.sin(4.*np.pi*xi[0]) * np.sin(2.*np.pi*xi[1]) * np.sin(4.*np.pi*(t0 + 0.3))
        # b_values[2*i]     = np.sin(4.*np.pi*xi[0]) * np.sin(2.*np.pi*xi[1]) * np.sin(4.*np.pi*(t.value-0.1))
        # b_values[2*i + 1] = np.sin(4.*np.pi*xi[0]) * np.sin(2.*np.pi*xi[1]) * np.sin(4.*np.pi*(t.value+0.3))
    b.x.scatter_forward()
